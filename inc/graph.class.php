...
*/
   ...
   **/
   ...
   function showPluginFromSupplier($ID,$withtemplate='') {
      ...
      if ($withtemplate!=2) echo "<form method='post' action=\"".Plugin::getPhpDir("archimap_plus")."/front/graph.form.php\">";
...
//      echo "<th>".PluginArchimapGraphCategory::getTypeName(1)."</th>";
      ...
      while ($data=$DB->fetch_array($result)) {
         ...
         if ($withtemplate!=3 && $canread && (in_array($data['entities_id'],$_SESSION['glpiactiveentities']) || $data["is_recursive"])) {
            echo "<td class='center'><a href='".Plugin::getPhpDir("archimap_plus")."/front/graph.form.php?id=".$data["id"]."'>".$data["name"];
